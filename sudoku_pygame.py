...
board, r_board = sudoku_func.generate(REMOVE)
hint_board = sudoku_func.get_hint_board(r_board)

#func definition

def draw_num(hint, hint_board):
    xs = XS 
    ys = YS
    ...
    if hint:
        xs = 5
        ys = 5
        for line in hint_board:
            for num in line:
                if type(num) == list:
                    origin_xs = copy.deepcopy(xs)
                    original_ys = copy.deepcopy(ys)
                    for i, n in enumerate(num):
                        if i % 3 == 0 and i != 0:
                            xs = origin_xs
                            ys += YSPACE//3
                        text = hint_font.render(str(n), 1, RED)
                        screen.blit(text, (xs, ys))
                        xs += XSPACE//3
...
def draw(valid, rec_x, rec_y, guess_right, notify, mistakes, nums_pos, win, hint, hint_board):
    screen.fill(BLACK)
    #drawing grid
...
def main():
    ...
    while run:
        ...
        win = check_win()
        draw(valid, rec_x, rec_y, guess_right, notify, mistakes, nums_pos, win, hint, hint_board)
